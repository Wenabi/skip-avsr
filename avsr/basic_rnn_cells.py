...
class BasicLSTMCell2(tf.contrib.rnn.RNNCell):
    """
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope or type(self).__name__):
            print('BasicLSTMCell_state', state)
            c, h = state
            ...
            new_c = clip_ops.clip_by_value(new_c, -1.0, 1.0)
            
            new_h = self._activation(new_c) * tf.sigmoid(o)
            print('BaiscLSTMCell_new_h', new_h)
            print('BaiscLSTMCell_new_c', new_c)
...
class BasicLSTMCell(tf.contrib.rnn.LSTMCell):
